...
const Register: React.FC = () => {
  ...
  const { register, handleSubmit, setValue, reset } = useForm<RegisterFormData>();
  
  const handleRegister = async (data: RegisterFormData): Promise<void> => {
    const { Confirmpassword, ...filteredData } = data;
    try {
      const resultAction = await dispatch(registerUser(filteredData) as any).unwrap();
      reset();
      navigate("/login");
    } catch (error) {
  ...
  return (
    <div className="   w-full max-w-[1200px] m-auto mt-10 md:h-full font-Josefin flex items-center justify-center overflow-hidden  ">
      <div className="w-full border-2   h-full mb-10 bg-white rounded-lg md:shadow-[0px_3px_6px_rgba(0,0,0,0.16),_0px_3px_6px_rgba(0,0,0,0.23)] flex  justify-center ">
        {/* Left Side: Form */}
        <div className=" border-4 w-full md:w-1/2  lg:w-1/3 p-6 md:p-12 flex flex-col justify-center ">
          <div className="mb-8 ">
            <img
              src="/logo.png"
              alt="Workflow"
              ...
              {selectedOption.index != null && (
                <div key={RegisterFields[selectedOption.index].id} className="">
                  ...
                  <input
                    ...
                    required
                    {...register(RegisterFields[selectedOption.index].name as string)}
